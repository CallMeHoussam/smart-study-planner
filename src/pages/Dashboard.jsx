...
const Dashboard = ({ tasks, setTasks }) => {
  ...
  useEffect(() => {
    ...
    const savedFocusMinutes = JSON.parse(localStorage.getItem("focusMinutes")) || 0;
    const savedCompletedTasks = JSON.parse(localStorage.getItem("completedTasks")) || 0;
    const savedCategories = JSON.parse(localStorage.getItem("categories")) || ["General", "Study", "Work", "Personal"];

    setTasks(savedTasks);
    setTotalFocusMinutes(savedFocusMinutes);
    setCompletedTasksCount(savedCompletedTasks);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("completedTasks", JSON.stringify(completedTasksCount));
    localStorage.setItem("categories", JSON.stringify(categories));
  }, [tasks, totalFocusMinutes, completedTasksCount, categories]);

  const filteredTasks = tasks.filter(task => 
    task.title.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const currentCompletedCount = tasks.filter(task => task.completed).length;
  ...
  const handlePomodoroComplete = (minutesFocused) => {
    setTotalFocusMinutes(totalFocusMinutes + minutesFocused);
  };
  ...
  const calculateProductivityScore = () => {
    ...
    return Math.round((currentCompletedCount / tasks.length) * 100);
  };

  const totalPlannedFocusHours = tasks.reduce((total, task) => total + (task.focusHours || 0), 0);
  const completedFocusHours = tasks
    .filter(task => task.completed)
    .reduce((total, task) => total + (task.focusHours || 0), 0);

  const handleAddCategory = () => {
    if (newCategoryName.trim() && !categories.includes(newCategoryName.trim())) {
      const updatedCategories = [...categories, newCategoryName.trim()];
      setCategories(updatedCategories);
      setNewTask({ ...newTask, category: newCategoryName.trim() });
      setNewCategoryName("");
  ...
  const handleCustomFocusHours = () => {
    const hoursInput = prompt("Enter custom focus hours:");
    const hours = parseInt(hoursInput);
    if (hours && !isNaN(hours) && hours > 0) {
      setNewTask({ ...newTask, focusHours: hours });
    }
  };
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="flex items-center space-x-3">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-2 shadow text-sm">
              ...
              <span className="font-semibold text-blue-600 dark:text-blue-400">{totalFocusMinutes}min</span>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg p-2 shadow text-sm">
              <span className="text-gray-600 dark:text-gray-300">Tasks: </span>
              <span className="font-semibold text-green-600 dark:text-green-400">{currentCompletedCount}/{tasks.length}</span>
            </div>
            <button 
        ...
        {showStatistics && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-3 bg-green-50 dark:bg-green-900/30 rounded-lg">
              ...
              <div className="text-2xl font-bold my-2 text-gray-800 dark:text-white">{totalFocusMinutes} min</div>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {totalFocusMinutes > 0 ? `~${Math.round(totalFocusMinutes/60)} hours` : "Start a session!"}
              </p>
            </div>
            
            <div className="text-center p-3 bg-purple-50 dark:bg-purple-900/30 rounded-lg">
              <h3 className="text-base font-semibold text-purple-700 dark:text-purple-300">Focus Goals</h3>
              <div className="text-2xl font-bold my-2 text-gray-800 dark:text-white">{completedFocusHours}/{totalPlannedFocusHours}h</div>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {totalPlannedFocusHours > 0 ? `${Math.round((completedFocusHours/totalPlannedFocusHours) * 100)}% completed` : "No focus goals"}
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === "tasks" && (
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
              ...
              <form onSubmit={handleAddTask} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                <input
                  type="text"
              ...
              <div className="flex justify-between items-center mb-3">
                <h4 className="text-base font-medium text-gray-800 dark:text-white">
                  Your Tasks ({tasks.length - currentCompletedCount} remaining)
                </h4>
              ...
              <div className="max-h-96 overflow-y-auto">
                <TaskList
                  tasks={filteredTasks}
                  toggleTaskCompletion={handleTaskCompletionToggle}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-blue-600 dark:text-blue-400">{totalFocusMinutes} minutes</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-300">Focus Goals:</span>
                    <span className="font-medium text-green-600 dark:text-green-400">{completedFocusHours}/{totalPlannedFocusHours}h</span>
                  </div>
                </div>
              </div>
            </div>
                          ...
                          return (
                            <div key={category} className="flex justify-between items-center text-sm">
                              ...
                              <span className="font-medium text-gray-800 dark:text-white">
                                {totalHours}h ({completedTasks}/{categoryTasks.length} tasks)
                              </span>
                            </div>
                          );
                        })}
                ...
                <div className="bg-green-50 dark:bg-green-900/30 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-green-800 dark:text-green-200">{currentCompletedCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
            ...
            <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
              ...
              <p className="text-xs text-gray-700 dark:text-gray-300">
                You've focused for {totalFocusMinutes} minutes total. 
                {totalFocusMinutes >= 120 ? " Great job!" : " Keep going!"}
              </p>
            </div>
          </div>
        )}
